...
public class MonitorApi {
    ...
    public List<MonitorMetricResponse> getMonitorMetric(MonitorRequestParams monitorRequestParams) {
        if (Objects.isNull(monitorRequestParams.getId()) || StringUtils.isBlank(
                monitorRequestParams.getMonitorType())) {
            log.error("Parameters Invalid,id: {},getMonitorType: {}", monitorRequestParams.getId(),
                    monitorRequestParams.getMonitorType());
            throw new IllegalArgumentException("Parameters Invalid");
        }
        return orchestratorService.getMonitorMetric(monitorRequestParams);
    }
